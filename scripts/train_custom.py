...
def sparse_cce_with_label_smoothing(label_smoothing: float = 0.1):
    def loss(y_true, y_pred):
        y_true = tf.cast(y_true, tf.int32)
        num_classes = tf.shape(y_pred)[-1]

        y_true_oh = tf.one_hot(y_true, num_classes)  # (batch, num_classes)
        smooth_pos = 1.0 - label_smoothing
        smooth_neg = label_smoothing / tf.cast(num_classes, tf.float32)

        y_true_smoothed = y_true_oh * smooth_pos + smooth_neg
